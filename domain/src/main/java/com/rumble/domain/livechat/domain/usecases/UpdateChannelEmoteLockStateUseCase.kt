...
class UpdateChannelEmoteLockStateUseCase @Inject constructor() {
    operator fun invoke(emoteGroups: List<EmoteGroupEntity>?, followsCurrentChannel: Boolean, subscribedToChannel: Boolean) =
        emoteGroups?.map { group ->
            group.copy(
                emoteList = group.emoteList.map { emoteEntity ->
                    emoteEntity.copy(
